                    ...
                    <div  @click.away="show = false" class="z-50 relative p-3 mx-auto my-0 max-w-full" style="width: 600px;">
                        <div class="bg-white rounded shadow-lg border overflow-hidden">
                            ...
                            <div class="bg-white w-full  p-4 border-gray-100 shadow-xl rounded-lg">
                                ...
                                <form action="{{ route('nominas.store') }}" method="POST" class="">
                                    @csrf 
                                    ...
                                    <div class="">
                                        ...
                                        placeholder="Periodo" name="periodo">
                                            <option>---Seleccione Periodo--</option>
                                            <option>Enero-Agosto 2021</option>
                                            <option>Agosto-Diciembre 2021</option>
                                            <option>Enero-Agosto 2022</option>
        ...
        <div class="bg-white mx-auto overflow-hidden shadow-xl sm:rounded-lg w-max ">
            ...
            <table class="table-fixed w-max">
                ...
                <tr class="bg-indigo-500 text-white w-max">
                    <th class="w-20 p-4 ...">ID Empleaado</th>
                    <th class="w-20 p-4...">Periodo</th>
                    <th class="w-30 p-4 pr-4 ...">Actividades</th>
                    <th class="w-30 p-4..."> Departamento</th>
                    <th class="w-20 p-4...">Folio</th>
                    <th class="w-10 p-4...">Total de Horas</th>
                    <th class="w-28 p-4 ...">Actions</th>
                </tr>
                </thead>
                <tbody>
                    @foreach ($nominas as $row)
                        ...
                        <tr>
                            ...
                            <td class="p-3">{{$row->periodo}}</td>
                            <td class="p-3">{{$row->actividades}}</td>
                            <td class="p-3">{{$row->departamento}}</td>
                            <td class="p-3">{{$row->folio}}</td>
                            <td class="p-3">{{$row->t_horas}}</td>
                            <td class="p-3 flex justify-center">

                            <form action="{{route('nominas.destroy',$row->id)}}" method="POST">
                                @csrf 
                                @method('delete')
                            ...
                            <a href="{{route('nominas.edit',$row->id)}}" class="bg-green-500 text-white px-3 py-1 rounded-sm">
